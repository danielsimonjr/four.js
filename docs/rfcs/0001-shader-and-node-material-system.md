...
**Residue (not an MVP reopen).** Both emitters now prefix each reachable local with `// node <index> <kind>` — deterministic provenance so a `SHADER_COMPILATION_FAILED` driver log can be correlated with graph nodes. That is not a source map (no file:line table); the source-maps row above stays deferred, as do uniform blocks, reusable functions, conditional variants, storage buffers, and lighting-aware graphs.
