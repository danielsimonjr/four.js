...
- the tooling **examples** row claimed "10 example applications … incl. the five §93 guide
  ...
  said six until 2026-08-07 and says seven since (`first-3d-scene` was written); the
  remaining shortfall is staged as **S-8**;
...
| examples                       | shipped (MVP tier) | **7** runnable example applications under `examples/`: `first-2d-scene`, `first-3d-scene`, `physics-playground`, `mechanism`, `blending`, `particles-demo`, `ui-demo` — each with a `main.ts`, a Vite build script and a browser spec           | This row said "10 example applications … incl. the five §93 guide scenes and the flagship" until 2026-08-05. It was never true: `examples/{first-3d-scene,first-animated-scene,first-physics-scene,mixed-scene}` and both `examples/flagship/*` directories contain only a `.gitkeep` (verify with `git ls-files examples/`). The count read **6** until 2026-08-07, when `first-3d-scene` was written and its directory stopped being a placeholder. §120 asks for "examples" without a count, and seven runnable ones satisfy the row; the remaining §93/§118–119 shortfall is staged as **S-8** |
...
partially closed 2026-08-07.** **Five** directories under `examples/` contain a `.gitkeep`
and nothing else: `first-animated-scene`, `first-physics-scene`, `mixed-scene`,
...
scene. So three of §93's six quick-start items and both §118–119 flagships have no
implementation.
