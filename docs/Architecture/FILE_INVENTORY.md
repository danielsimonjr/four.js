...
| --- | --: | --- |
| `reachable` | 185 | A `src/` file in the module graph, reachable from a root. |
...
| `test` | 310 | A test source file (under a `tests/` dir, or a `*.test.ts`/`*.spec.ts`). |
