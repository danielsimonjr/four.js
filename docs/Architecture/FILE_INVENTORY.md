...
| --- | --: | --- |
| `reachable` | 108 | A `src/` file in the module graph, reachable from a root. |
...
| `test` | 153 | A test source file (under a `tests/` dir, or a `*.test.ts`/`*.spec.ts`). |
...
| `packages/motion/tests/random.test.ts` | @four/motion | tests | test |
