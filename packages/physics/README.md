...
Implements §20–§34 and §37 (§101, Part IV) of [`docs/SPECIFICATION.md`](../../docs/SPECIFICATION.md); shipped across Phases 5–7. Application code targets this package, never a solver directly — a concrete engine plugs in through `PhysicsSolverAdapter` (see `@fourjs/physics-rapier`). Y-up gravity, seconds, radians; 2D bodies sit on the z = 0 plane (§21).
...
- **World** — `PhysicsWorld` / `PhysicsSystem`: fixed-step solve, event dispatch after each step (§29 collision/trigger/sleep events), snapshots (§34), and §30 queries (raycast, shape cast, overlap, point) with filter semantics. `world.forEachActiveBody(visit)` walks every dynamic, awake body in registration order (§33) with its world-space centre of mass — the iteration a §39 step-5 force generator needs. `world.forEachSleepingDynamicBody` is the complementary walk for a field that opts into `wakesSleepingBodies`.
- **Force fields (§26, §27)** — `ForceField` (§27's interface, transcribed) and `ForceFieldSystem`, the engine occupant of §39 step 5. It samples every registered field at every active body each fixed step and applies the sum through §26's `applyForce`; optional `sampleTorque` adds N·m through `applyTorque` (linear `units` do not scale it). `world.step` is not involved, so an application that does not register it issues the solver-call sequence it always did. Units are declared per field, `"force"` (N) or `"acceleration"` (m/s², multiplied by the body's mass) — a **required** argument, because §27's own built-in list mixes the two. Per-entry `wakesSleepingBodies` (default off) is the only way a field visits a sleeper; a sibling without the flag still cannot. `ParticleForceField` from `@fourjs/particles` is structurally identical, so every built-in field there (`uniformGravityField`, `radialField`, `vortexField`, `windField`, `dragField`, `turbulenceField`, `volumeField`) works here with no adapter and no dependency edge.
- **§8 space modes** — `RigidBody.space` declares the frame a body is solved in (`@fourjs/core`'s `SpaceMode`; default `"world"`, round-trips through §79). `PhysicsWorld.addBody` accepts `"world"` and `"local-plane"` (mapped through `PhysicsWorld.localPlane`, default XY) and refuses the four presentation frames because §8 forbids screen-space content from automatically participating.
- **§40 units** — optional `PhysicsWorldOptions.units` (`scale.lengthToMeters` / `massToKilograms`). When set, authored gravity / poses / masses convert into SI for the solver; omitted is today's identity path. Helpers: `world.toSiLength` / `fromSiLength` / `toSiMass` / `fromSiMass`.
- **Live velocities (PH-1)** — `world.setLinearVelocity` / `setAngularVelocity` write the solver immediately under `"physics"` authority (§42). Static bodies refuse; any other authority warns once and does not write.
...
- §27's field **torque** channel and field-driven waking shipped 2026-09-06 (`ForceField.sampleTorque`, per-entry `wakesSleepingBodies`). Built-in particle fields still contribute no torque.
- Character / dynamics push shipped (PH-11c): `SweptCharacterController.pushDynamics` (default on), `pushMass` (80 kg), `pushImpulseScale`.
...
Workspace name `@fourjs/physics`; publishes as `@danielsimonjr/fourjs-physics`.
