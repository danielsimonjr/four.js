 ...
 * (A-5, 2026-09-06).
 *
 * {@link auditResourceLeaks} answers a *span* question: did these live-instance
 * counters climb between two readings? This module answers a *lifetime*
 * question: was a disposable constructed, dropped, and then garbage-collected
 * without `dispose()`?
 *
 ...
 * (or an explicit `creationSite`) at construction. The two sit side by side
 * and neither imports `@four/geometry` or `@four/render` — callers pass the
 * object they own.
 *
 ...
 * {@link trackDisposable} returned, which is the same function the registry
 * uses as its held-value callback.
 *
 * ```ts
 * import { trackDisposable, disposeTracked, auditFinalizedLeaks } from "@four/diagnostics";
 *
 ...
 * {@link @four/core!resetDevWarnings | resetDevWarnings}.
 */
