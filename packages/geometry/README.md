...
- **Tessellation** (§52) — `triangulatePolygon` (ear clipping with bridged holes) behind the `PolygonTessellator` seam.
...
- **Offset paths, and Boolean geometry operations** (union, intersection, subtraction, xor) — §51's remaining four operations. The offset needs §58's join model (gap `R-16`); the booleans need the planar subdivision §52's self-intersection support also needs, and the two should be built once, together.
- **Stroke expansion and anti-alias fringe generation** — §52's other half, with `R-16`.
