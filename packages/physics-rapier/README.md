...
- **`Rapier2dAdapter`** and **`Rapier3dAdapter`** — one per §21 dimension, each wrapping its own `@dimforge/rapier{2,3}d-compat` build (pinned 0.20.0, base64-inlined wasm). Neither loads the other's module; importing the barrel costs nothing until `initializeRapier2d` / `initializeRapier3d` runs (async, §37 permits a Promise).
...
- Rapier 0.20.0 exposes no joint reaction getters → `reportsJointReactions` is false and breakable joints are refused; motor `maxTorque`/`maxForce` acts as a force-based gain, not §28's hard cap.
