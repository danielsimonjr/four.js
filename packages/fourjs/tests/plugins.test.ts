 ...
 *
 * `@fourjs/core`'s `plugin.test.ts` owns the machinery — order, ranges, refusals,
...
import { RendererRegistry } from "@fourjs/render";
...
describe("capability tokens", () => {
  it("re-exports the owning packages' very objects — identity, not copies (RFC 0002 §2)", async () => {
    ...
    const motion = await import("@fourjs/motion");
    const physics = await import("@fourjs/physics");
    const render = await import("@fourjs/render");
    const serialization = await import("@fourjs/serialization");
    const assets = await import("@fourjs/assets");
    const materials = await import("@fourjs/materials");
