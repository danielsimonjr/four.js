 ...
 *
 * `@fourjs/core` owns the plugin *machinery* — {@link @fourjs/core!FourPlugin},
 * {@link @fourjs/core!PluginHost}, {@link @fourjs/core!defineCapability} — and
 ...
 * |---|---|---|
 * | `SIMULATION_SYSTEMS` | `@fourjs/motion` | the §39 `SystemRegistry` |
 * | `RENDERER_REGISTRY` | `@fourjs/render` | the §62 `RendererRegistry` |
 * | `RENDER_GRAPH` | `@fourjs/render` | the application's §63 `RenderGraph` |
 * | `COMPUTE_WORKLOADS` | `@fourjs/render` | a named `ComputePassDescriptor` factory table |
 * | `SOLVER_REGISTRY` | `@fourjs/physics` | the §37 `SolverRegistry` |
 * | `COMPONENT_SERIALIZERS` | `@fourjs/serialization` | the §79 registry |
 * | `SCENE_MIGRATIONS` | `@fourjs/serialization` | the §80 upgrade chain |
 * | `ASSET_LOADERS` | `@fourjs/assets` | a named `AssetLoader` table |
 * | `SHADER_OPERATORS` | `@fourjs/materials` | a named shader-operator factory table |
 * | `UI_CONTROLS` | `@fourjs/ui` | a named widget-constructor table |
 ...
 * `import { SIMULATION_SYSTEMS } from "fourJS"` and
 * `import { SIMULATION_SYSTEMS } from "@fourjs/motion"` compare `===`, every
...
export { COMPONENT_SERIALIZERS, SCENE_MIGRATIONS } from "@fourjs/serialization";
export { UI_CONTROLS } from "@fourjs/ui";
