 ...
 *
 * `@fourjs/diagnostics` cannot import `@fourjs/geometry`, `@fourjs/render`,
 * `@fourjs/materials`, or `@fourjs/physics` (plan §3.1), so the audit takes a
...
import type { LiveResourceCounts } from "@fourjs/diagnostics";
import { geometryMemoryBytes, liveGeometryCount } from "@fourjs/geometry";
