...
- **JSON utilities** — `JsonValue` guards (`isJsonObject`, `isJsonArray`, `asJsonObject`) and `cloneJsonValue` (re-exported from `@fourjs/core`, `__proto__`-refusing).
...
- §79 documents deliberately do not carry solver warm-start state — a contact-free save replays bit-identically, an in-contact save diverges only through warm-start; §34 snapshots (`@fourjs/diagnostics`) carry that state.
- The binary `.four` format is not implemented; the JSON document is the shipped format.

Unit tests are colocated in `tests/` per §92.

Workspace name `@fourjs/serialization`; publishes as `@danielsimonjr/fourjs-serialization`.
