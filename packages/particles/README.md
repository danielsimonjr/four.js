...
- **`ParticleEmitter`** — spawn rate, bursts (`ParticleBurst`), ranges/ramps (`ParticleRange`, `ParticleLifetimeRamp`), seeded via `SeededRandom` (re-exported from `@fourjs/core`).
...
- **`ParticleRenderable`** — bridges to `@fourjs/render`'s instanced particle path (`PARTICLE_INSTANCE_FLOATS` stride-8 interleaved instances; the `ParticleDrawable` contract is deliberately duck-typed — the dependency matrix forbids the edge).
...
Workspace name `@fourjs/particles`; publishes as `@danielsimonjr/fourjs-particles`.
