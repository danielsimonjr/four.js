/**
 * Seeded pseudo-random numbers for deterministic motion (§33, plan P8-3).
 *
 ...
 * explicit, injectable stream — which is what {@link SeededRandom} is.
 *
 * Plan P8-3 asked for the seeded RNG in `@four/core` "else a small xorshift
 * utility in motion with a documented seed contract". **`@four/core` has none**
 * (verified 2026-08-02: `packages/core/src` is `component`, `disposable`,
 * `errors`, `events`, `index` — no RNG, and nothing in any package exports one),
 * so this is that utility. It lives in `@four/motion` because §111's wander is
 * its only current consumer; if a second pillar needs it, moving it down to
 * `@four/core` is a mechanical change (this module imports nothing).
 *
 * ## The generator (decision, WP-8.2)
 *
 ...
 *   per cycle. At one draw per agent per fixed step, a 10 000-agent scene at
 *   60 Hz would take on the order of 10²⁶ years to wrap.
 ...
 *
 * xorshift32 (period 2³² − 1) was rejected: 4.3 × 10⁹ draws is within reach of a
 * long soak test, and its low bits are visibly weak in a 2D random walk, which
 * is exactly what wander-driven motion is. SplitMix64 was rejected
 * because JavaScript has no fast 64-bit integer multiply — emulating one costs
 * more than four xors and three shifts, and `BigInt` on a hot path is out of the
 * question.
 *
 * ## Seeding (decision, WP-8.2)
 *
 ...
 * ```
 *
 * The two multipliers are pinned *here*: the formula in this file is the
 * contract, not any external reference, and nothing downstream may depend on
 * them beyond "the same seed gives the same stream". Expanding through an
 * avalanche rather than writing the seed straight into `x` matters — raw
 * xorshift state with 127 zero bits needs dozens of draws to decorrelate, so
 * seeds `1`, `2`, `3` would produce visibly similar first outputs.
 *
 ...
 *    on how many draws *it* has served. Give each agent its own stream (or a
 *    documented shared one) and the scene reproduces regardless of iteration
 ...
 *    that varies.
 * 5. **Draw order is the contract.** Adding a draw to a behaviour changes every
 *    subsequent number, exactly as it would for any other PRNG. Determinism
 *    goldens are therefore sensitive to behaviour changes by design (§33).
 *
 * `tests/random.test.ts` pins the first eight outputs and the thousandth output
 * for five seeds against an independent BigInt implementation of the formulas
 * above — a genuinely separate arithmetic path, not a re-run of this code.
 */
 ...
 * ```
 *
 * See the module documentation for the generator, the seeding rule, and the
 * five-point determinism contract. Drawing allocates nothing; only the
...
export class SeededRandom {
   ...
   *
   * Not validated — this is a hot path (wander draws from it every fixed step).
   * `min === max` returns `min`; `max < min` yields `(max, min]`, the same
   * interval traversed downwards; a non-finite bound propagates to the result.
   * Rounding can make the result equal `max` only when `max - min` is so large
   * that `min + (max - min) · x` rounds up at `x` just below 1, i.e. never for
   * ranges of ordinary magnitude.
   */
