 ...
 * `@fourjs/render` `PickingService` presented as `@fourjs/input`'s structural
 * `PickProvider`.
 *
 * It lives here because the umbrella is the one layer that may name both
 * sides: `@fourjs/input` may not import a render type (plan §3.1 — the whole
 * reason the seam exists), and `@fourjs/render` has no business knowing what a
...
import type { PickProvider } from "@fourjs/input";
import type { PickingService } from "@fourjs/render";
import type { Viewport } from "@fourjs/scene";
