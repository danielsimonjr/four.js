 ...
 *
 * `@fourjs/assets` already resolves a logical key to verified bytes
 ...
 *
 * It lives here, not in `@fourjs/assets`, for the same reason `instantiateGltf`
...
> extends ManifestLoadOptions {
   ...
   * a `Texture` here — `@fourjs/assets` cannot name `@fourjs/render`, which
