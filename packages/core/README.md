...
- **`SeededRandom`** — xorshift128 with splitmix32 seeding; the deterministic RNG the §33–34 machinery relies on (`@fourjs/motion` and `@fourjs/particles` re-export it).
...
- §101's _"unit application in simulation"_ — `@fourjs/physics` reading `scale.lengthToMeters` for the §41 precision envelope (2026-08-07).
...
Workspace name `@fourjs/core`; publishes as `@danielsimonjr/fourjs-core`.
