...
- **`Label`** — text measured with `@fourjs/text`'s glyph atlas.
...
- **`collectPickables`** — bridges a widget tree into `@fourjs/input`'s §71 picking.
- **Keyboard navigation (§75)** — `installKeyboardTraversal` (Tab / Shift-Tab over the widget tree), `collectFocusOrder` (scene order, sorted by `accessibility.tabIndex`; negative opts out), and `keyboardFocusTarget` — the resolver `@fourjs/input`'s `KeyboardInput` takes, which is how keys reach the focused widget without `input` ever depending on `ui`.
...
Workspace name `@fourjs/ui`; publishes as `@danielsimonjr/fourjs-ui`.
