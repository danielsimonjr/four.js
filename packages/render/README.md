...
- **`Sprite` and `Texture`** — the §55/§77 MVP tier (`TextureSource` accepts what `@four/text`'s glyph atlas emits). `Sprite.frame` is the atlas cell; `groupSpritesByTexture` reports consecutive same-texture runs. Non-2D `TextureSource.dimension` values are refused with `NOT_IMPLEMENTED` (R-30c).
