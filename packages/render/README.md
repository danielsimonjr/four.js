...
- **`Sprite` and `Texture`** — the §55/§77 MVP tier (`TextureSource` accepts what `@fourjs/text`'s glyph atlas emits). `Sprite.frame` is the atlas cell; `groupSpritesByTexture` reports consecutive same-texture runs. Non-2D `TextureSource.dimension` values are refused with `NOT_IMPLEMENTED` (R-30c).
- **Particle contract** — `ParticleDrawable` (duck-typed toward `@fourjs/particles`; the dependency matrix forbids the direct edge), `particleQuadGeometry`, and the stride-8 instance layout constants (`PARTICLE_INSTANCE_FLOATS` and offsets).
...
Workspace name `@fourjs/render`; publishes as `@danielsimonjr/fourjs-render`.
