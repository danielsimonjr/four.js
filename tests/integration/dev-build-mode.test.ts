...
const GATED: ReadonlyMap<string, string> = new Map([
  ...
  [
    ...
    "§71's id-pipeline compile-failure notices (RFC 0005, 2026-09-09) — the WebGPU twin of gl-picking.ts's entry: mesh, particle, and skinned id pipelines each latch a per-device failure in both builds; picking passes (or just the skinned arm) are skipped identically, never a bind-pose silhouette, and only the console.warn carrying §89's log moves with the flag. Picking is outside the §33 envelope regardless: a pick is a §34 *input*, its result never enters a checksum, and nothing an id pass draws re-enters simulation state (§42/§43)",
  ],
  [
